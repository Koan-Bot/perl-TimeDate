[![Actions Status](https://github.com/atoomic/perl-TimeDate/actions/workflows/ci/badge.svg)](https://github.com/atoomic/perl-TimeDate/actions)
...
- str2time(DATE \[, ZONE\])
    ...
    `ZONE`, if given, specifies the timezone to assume when parsing if the
    date string does not specify a timezone.

- strptime(DATE \[, ZONE\])

    `strptime` takes the same arguments as str2time but returns an array of
    values `($ss,$mm,$hh,$day,$month,$year,$zone,$century)`. Elements are only
    defined if they could be extracted from the date string. The `$zone` element
